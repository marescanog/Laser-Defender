...
public class HandMinion : MonoBehaviour
{
    [Header("Pathing")]
    ...
    void Start()
    {
        ...
        orbLeft = GameObject.Find("Orb Left");
        ...
        {
            bossBrainy = GameObject.Find("Boss Brainy(Clone)"); //would depend if Placed there
            //If spawned by the enemy spawner then (Clone) at the end
            //but if just placed in the scene no (Clone) at the end
        }

        //Initializing Values
        mySpriteRenderer.color = transparentColor;
        enableShooting = false;
        gameSessionScript.Turn_ON_GamesessionBool_is_LeftHandMinion_in_Scene();
        ///
    ...
    void Update()
    {
        ...
        if(keep_looking_for_boss)
        {
            ...
            {
                bossBrainy = GameObject.Find("Boss Brainy");
                keep_looking_for_boss = false;
            }
        }
    ...
    private void Die()
    {
        ...
        if (addOnce)
        {
            ...
            orbLeft.GetComponent<OrbScript>().ResetLeftHandSpawnerTimer();
            addOnce = false;
            gameSessionScript.Turn_OFF_GamesessionBool_is_LeftHandMinion_in_Scene();
        }

        Instantiate_Explosion_On_Death_HandMinion_Left();

        Destroy(gameObject);

    }

    public void Instantiate_Explosion_On_Death_HandMinion_Left()
    {
    ...
    private void Movement()
    {
        if (phaseInDone)
        {
            ...
            if(keep_looking_for_boss)
            {
                myTransform.position = orbLeft.transform.position;
            }
    ...
    public void DestroyLeftHandMinion() { detroyWhilePhasing = true; }
